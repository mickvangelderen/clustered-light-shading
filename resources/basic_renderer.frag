...
void main() {
  ...
  // frag_color = vec4(vec3(frag_depth_in_lgt), 1.0);
  float closest_depth_in_lgt =
      texture(shadow_sampler, fs_pos_in_lgt.xy * 0.5 + 0.5).x;
  // frag_color = vec4(vec3(closest_depth_in_lgt), 1.0);
  frag_color = vec4(10 * vec3(frag_depth_in_lgt - closest_depth_in_lgt), 1.0);
  float bias = 0.002;
  float in_shadow = step(frag_depth_in_lgt, closest_depth_in_lgt + bias);
  frag_color = vec4(
